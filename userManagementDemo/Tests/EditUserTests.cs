...
public class EditUserTests 
{
    ...
    [Test, Category("Edit User")]
    [TestCase(TestName = "Verify user can able to edit")]
    public async Task EditUser_Test()
    {
        ...
        EditUserPage editUserPage = new EditUserPage(_page);

        await editUserPage.editUser(Constants.FIRSTNAME);

        await Task.Delay(10000);

        Assert.That(await editUserPage.HasSaved(), Is.True);
    }
